...
function getGenericName(medicineName) {
  // Extract the main component (before the strength/dosage)
  const parts = medicineName.split(' ');
  return parts[0]; // e.g., 'Paracetamol' from 'Paracetamol 500mg'
}
...
async function importMedicines() {
  try {
    ...
    if (!manufacturer) {
      ...
      console.log('✅ Using existing default manufacturer\n');
    }
    ...
    for (const med of medicineData) {
      try {
        ...
        // Create batch with initial stock
        const batchNumber = `BATCH-${Date.now()}-${med.name.replace(/\s+/g, '-')}`;
        ...
        errorCount++;
        console.log(`❌ Error importing ${med.name}: ${error.message}`);
      }
    }
    ...
    console.log(`✅ Successfully imported: ${successCount} medicines`);
