...
async function importAll() {
  ...
  const supplier = await db.collection('suppliers').findOne({ name: 'Default Supplier' }) || 
                  (await db.collection('suppliers').insertOne({
                    ...
                    createdAt: new Date(),
                    updatedAt: new Date()
                  }), await db.collection('suppliers').findOne({ name: 'Default Supplier' }));

  const mfg = await db.collection('manufacturers').findOne({ name: 'Generic Manufacturer' }) ||
            (await db.collection('manufacturers').insertOne({
              ...
              createdAt: new Date(),
              updatedAt: new Date()
            }), await db.collection('manufacturers').findOne({ name: 'Generic Manufacturer' }));
  ...
  for (let i = 0; i < medicineData.length; i++) {
    ...
    try {
      ...
      let generic = await db.collection('generics').findOne({ name: genericName });
      if (!generic) {
        const res = await db.collection('generics').insertOne({
          name: genericName,
        ...
        generic = { _id: res.insertedId };
      }

      let medicine = await db.collection('medicines').findOne({ name: med.name });
      if (!medicine) {
        const res = await db.collection('medicines').insertOne({
          name: med.name,
          genericId: generic._id,
          manufacturerId: mfg._id,
          category: med.category,
          unitType: med.category.toLowerCase().replace('supplement', 'tablet').replace('powder', 'tablet'),
          rackLocation: med.rack,
          minStockLevel: parseInt(med.minMax.split('/')[0]),
          maxStockLevel: parseInt(med.minMax.split('/')[1]),
          gstRate: 12,
          discontinued: false,
          createdAt: new Date(),
          updatedAt: new Date()
        });
        medicine = { _id: res.insertedId };
      }

      const batchNumber = `B${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
      ...
      console.log(`[${i+1}/${medicineData.length}] ✅ ${med.name}`);
      success++;
    } catch (error) {
      console.log(`[${i+1}/${medicineData.length}] ❌ ${med.name}: ${error.message}`);
      failed++;
    }
  }
