...
async function cleanAndImport() {
  ...
  try {
    ...
    await db.collection('generics').deleteMany({});
    console.log('  ✓ Deleted all generics\n');

    // ===== IMPORT PHASE =====
    console.log('📦 Setting up suppliers and manufacturers...');
    ...
    console.log(`📥 Importing ${medicineData.length} medicines...\n`);
    ...
    for (let i = 0; i < medicineData.length; i++) {
      ...
      try {
        ...
        const medicine = { _id: mRes.insertedId };
        ...
        // Create StockLedger entry for initial purchase
        await db.collection('stockledgers').insertOne({
          ...
          remarks: `Initial stock import - Batch ${batchNumber}`,
          createdAt: new Date(),
          updatedAt: new Date()
        });

        success++;
        console.log(`  ✓ (${i + 1}/${medicineData.length}) ${med.name} | Cost: ₹${med.purchasePrice} → Sell: ₹${med.sellingPrice}`);
      } catch (err) {
        failed++;
        console.error(`  ✗ Failed to import ${med.name}: ${err.message}`);
      }
    }

    console.log(`\n${'='.repeat(70)}`);
    console.log(`✅ Import Complete!`);
    console.log(`  Total: ${success + failed} | Success: ${success} | Failed: ${failed}`);
